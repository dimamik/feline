defmodule Feline.Processors.VADProcessor do
  @moduledoc """
  ...
  @impl true
  def handle_frame(
        %InputAudioRawFrame{audio: audio, sample_rate: sr} = frame,
        :downstream,
        _push_fn,
        state
      ) do
    # Drop mic audio while bot is speaking to prevent echo/feedback
    if state.bot_speaking do
      {:ok, state}
    else
      ...
      out_frames = Enum.map(events, &{&1, :downstream})
      {:push_many, [{frame, :downstream} | out_frames], state}
    end
  end

  def handle_frame(%BotStartedSpeakingFrame{} = frame, dir, _push_fn, state) do
    {:push, frame, dir, %{state | bot_speaking: true}}
  end

  def handle_frame(%BotStoppedSpeakingFrame{} = frame, dir, _push_fn, state) do
    {:push, frame, dir, %{state | bot_speaking: false}}
  end
